...
if (count($update_fields) > 0) {
    ...
    // Check if the user already has an entry in the admin_details table
    $sql_check = "SELECT * FROM admin_details WHERE admin_id = '$user_id'";
    $result = mysqli_query($connection, $sql_check);

    if (mysqli_num_rows($result) > 0) {
        // If a record exists, update it
        $sql = "UPDATE admin_details SET $update_fields_str WHERE admin_id = '$user_id'";
    } else {
        // If no record exists, insert a new one
        $sql = "INSERT INTO admin_details (admin_id, address, date_of_birth, place_of_birth, nationality, sex, mobile_number, personal_email)
                VALUES ('$user_id', 
