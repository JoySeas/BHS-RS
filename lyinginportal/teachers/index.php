...
<body>
    <div class="container-fluid" style="padding: 15px 15px; background-color: white; min-height: 540px; margin-top: 15px; border-radius: 5px; border: 1px solid;">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h3 class="mb-0 headerfontfont" style="color: #2c2b2e; font-weight: 500;">Teachers</h3>
                    ...
                    <table data-height="350" class="table table-bordered fixTable table-hover" style="margin-bottom: 0px;">
                        <thead class="bg-success text-white">
                            <tr>
                                ...
                                <th style="width: 3%; white-space: nowrap;"> Teacher ID </th>
                                ...
                                <th style="width: 3%; white-space: nowrap; text-align: center;"> Action </th>
                            </tr>
                        </thead>
                        <tbody id="tblteacherlist"></tbody>
                    </table>
                </div>
...
<div id="contentModal" style="display: none;">
    <div style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.5); z-index: 999;">
        <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); background-color: #fff; padding: 20px; border-radius: 10px; width: 90%; max-width: 500px; max-height: 80%; overflow-y: auto; position: relative;">
            ...
            <div style="background-color: #5D9EFE; color: white; padding: 10px; position: relative; text-align: center; margin: -20px -20px 20px -20px;">
                <h2 style="margin: 0; color:#fff; font-weight: 500;">Teacher Details</h2>
                <!-- Close Icon -->
            </div>
            <div class="form-group">
                <div id="teacherImageDisplay" style="text-align: center;">
                    <img src="" alt="Teacher Image" id="teacherImage" style="max-width: 100px; height: auto; border-radius: 50%; border: 3px solid #5D9EFE;">
                </div>
            </div>
            <div class="form-group">
                <label for="teacherNameDisplay">Name:</label>
                <p id="teacherNameDisplay"></p>
            </div>
            <div class="form-group">
                <label for="teacherAddressDisplay">Address:</label>
                <p id="teacherAddressDisplay"></p>
            </div>
            <div class="form-group">
                <label for="teacherEmailDisplay">Email:</label>
                <p id="teacherEmailDisplay"></p>
            </div>
            <div class="form-group">
                <label for="teacherContactDisplay">Mobile Number:</label>
                <p id="teacherContactDisplay"></p>
            </div>
                    ...
                    $.each(data, function(index, teacher) {
                        rows += `<tr>
                            ...
                            <td style="text-align: center;">
                                <i class="fas fa-eye icon-size" data-id="${teacher.user_id}" title="View"></i>
                                <i class="fas fa-trash delete-icon icon-size" data-id="${teacher.user_id}" title="Delete"></i>
                            </td>
                        </tr>`;
                    });
                    $('#tblteacherlist').html(rows);
                },
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: 'teachers/delete_teacher.php',
                type: 'GET', // Ensure this matches your PHP script
                data: { user_id: teacherId }, // Send the teacher's user_id
        ...
        function openContentModal(user_id) {
    $.ajax({
        url: 'teachers/get_teacher.php', // Path to the PHP script that fetches teacher details
        type: 'GET',
        data: { id: user_id },
        dataType: 'json',
        success: function(data) {
            // Populate the modal with the fetched data
            $('#teacherImageDisplay img').attr('src', `../teacher/uploads/${data.profile_image}`);
            $('#teacherNameDisplay').text(data.name);
            $('#teacherAddressDisplay').text(data.address);
            $('#teacherEmailDisplay').text(data.email);
            $('#teacherContactDisplay').text(data.mobile_number);
...
$(document).on('click', '.fa-eye', function() {
    var teacherId = $(this).data('id'); // Ensure this matches the data attribute in your HTML
    openContentModal(teacherId);
});
// Fetch the total number of teachers from the server
function fetchTotalTeachers() {
        $.ajax({
            url: 'teachers/get_total_teachers.php', // Path to your PHP script
            type: 'GET',
            dataType: 'json',
            success: function(data) {
                // Update the total number of teachers on the page
                $('#teacherstotal').text(data.total);
            },
            error: function(xhr, status, error) {
                console.log("AJAX Error:", error);
            }
    ...
    // Fetch teacher data based on search input
    function searchTeachers(query) {
        $.ajax({
            url: 'teachers/search_teachers.php', // Path to your PHP script
            type: 'GET',
            data: { search: query },
            dataType: 'json',
            success: function(data) {
    ...
    // Fetch initial total number of teachers
    fetchTotalTeachers();
...
document.getElementById('exportPdfImage').addEventListener('click', function (event) {
    ...
    logo.src = 'teachers/JCCA-LOGO.png'; // Path to your logo image
    logo.onload = function() {
        ...
        pdf.setFontSize(12);
        pdf.text("Teachers Records", pageWidth / 2, 70, { align: 'center' }); // Center the subtitle
